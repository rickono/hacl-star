...
let inv_inner_inner h0 (z:zq) (len:len_t) (start:start_t len{v start + (2 * v len) <= 256}) (spec_res:erased (lseq PS.zq PS.n)) (res:rq) h (j:nat) =
    ...
    Seq.equal 
      ...
      (Seq.slice #_ #(PS.n) spec_res (v start + v len) (v len + j))

let add_v_lemma (a b:U32.t): Lemma (requires v a <= 256 /\ v b <= 256) (ensures v (a +. b) = v a + v b) = ()

val body_inner_inner_1:
    orig_seq:PS.lpoly PS.n
  -> old_seq:PS.lpoly PS.n
  -> new_seq:PS.lpoly PS.n 
  -> spec_seq:PS.lpoly PS.n
  -> z:zq
  -> new_j_v:zq
  -> new_j_plus_len_v:zq
  -> len:len_t
  -> start:start_t len{v start + (2 * v len) <= 256}
  -> j:U32.t{v start <= v j /\ v j < v (start +. len)}
  -> Lemma 
      (requires 
        Seq.equal (Seq.slice old_seq (v start) (v j)) (Seq.slice #_ #(PS.n) spec_seq (v start) (v j)) /\                    // the first part of the range matches the spec
        Seq.equal (Seq.slice old_seq (v j) (v start + v len)) (Seq.slice #_ #(PS.n) orig_seq (v j) (v start + v len)) /\    // the second part of the range matches original
        Seq.equal spec_seq (S.ntt_inner_inner_func (v z) orig_seq (v len) (v start)) /\                                     // the spec is the spec
        Seq.equal (Seq.upd (Seq.upd old_seq (v j) (v new_j_v)) (v (j +. len)) (v new_j_plus_len_v)) new_seq)                // the new seq is updated accordingly
      (ensures Seq.equal (Seq.slice new_seq (v start) (v j + 1)) (Seq.slice #_ #(PS.n) spec_seq (v start) (v j + 1)))
let body_inner_inner_1 new_seq spec_seq start fin = admit()

let body_inner_inner h0 (z:zq) (len:len_t) (start:start_t len{v start + (2 * v len) <= 256}) (spec_res:erased (lseq PS.zq PS.n)) (res:rq) (j:U32.t{v start <= v j /\ v j < v (start +. len)}): 
    Stack unit 
    (requires fun h -> inv_inner_inner h0 z len start spec_res res h (U32.v j) /\ Seq.equal spec_res (S.ntt_inner_inner_func (v z) (lpoly_v h0 res)  (v len) (v start)))
    (ensures fun h0' _ h1 -> inv_inner_inner h0 z len start spec_res res h0' (U32.v j) /\ inv_inner_inner h0 z len start spec_res res h1 (U32.v j + 1)) 
  =
    let h0' = ST.get () in
    let res_j = res.(j) in
    let res_j_plus_len = res.(j +. len) in
    let t = mul_zq z res_j_plus_len in
    let new_j_v = add_zq res_j t in
    let new_j_plus_len_v = add_zq res_j (neg_zq t) in
    let orig_seq: erased (lseq PS.zq PS.n) = lpoly_v h0 res in
    let old_seq: erased (lseq PS.zq PS.n) = lpoly_v h0' res in

    res.(j) <- new_j_v;
    let h0'' = ST.get () in
    let int_seq: erased (lseq PS.zq PS.n) = lpoly_v h0'' res in
    assert (Seq.equal (Seq.upd old_seq (v j) (v new_j_v)) int_seq);
    res.(j +. len) <- new_j_plus_len_v;
    let h1 = ST.get () in
    
    assert (v j < 256);
    let new_seq: erased (lseq PS.zq PS.n) = lpoly_v h1 res in
    // assert (Seq.equal spec_res (S.ntt_inner_inner_func (v z) orig_seq (v len) (v start)));
    // the first parts of the range now match the spec
    // seq_slice_idx_lemma (v j + v len) (v start + 2 * (v len)) (v j + v len) old_seq orig_seq;
    body_inner_inner_1 (lpoly_v h0 res) old_seq new_seq spec_res z new_j_v new_j_plus_len_v len start j;
    admit()
    // // assume (Seq.equal (Seq.slice (new_seq) (v start) (v j + 1)) (Seq.slice #_ #(PS.n) spec_res (v start) (v j + 1)));
    // assume (Seq.equal (Seq.slice #_ #(PS.n) (new_seq) (v start + v len) (v j + 1 + v len)) (Seq.slice #_ #(PS.n) spec_res (v start + v len) (v j + 1 + v len)));

    // // the latter parts of the range are untouched from the original
    // assume (Seq.equal (Seq.slice #_ #(PS.n) (new_seq) (v j + 1) (v start + v len)) (Seq.slice #_ #(PS.n) (lpoly_v h0 res) (v j + 1) (v start + v len)));
    // assume (Seq.equal (Seq.slice #_ #(PS.n) (new_seq) (v j + 1 + v len) (v start + 2 * (v len))) (Seq.slice #_ #(PS.n) (lpoly_v h0 res) (v j + 1 + v len) (v start + 2 * (v len))))

// let body_inner_inner_ h0 (z:zq) (len:len_t) (start:start_t len{v start + (2 * v len) <= 256}) (spec_res:erased (lseq PS.zq PS.n)) (res:rq) (j:U32.t{v start <= v j /\ v j < v (start +. len)}): 
//     Stack unit 
//     (requires fun h -> inv_inner_inner h0 z len start spec_res res h (U32.v j))
//     (ensures fun h0' _ h1 -> inv_inner_inner h0 z len start spec_res res h0' (U32.v j) /\ inv_inner_inner h0 z len start spec_res res h1 (U32.v j + 1)) 
//   =
//     let h0' = ST.get () in
//     let res_j = res.(j) in
...
//     let h1 = ST.get () in
//     assert (Seq.index (lpoly_v h1 res) (v j) == Seq.index #_ #(PS.n) spec_res (v j));
//     assert (Seq.equal (Seq.slice (lpoly_v h1 res) (v start) (v j)) (Seq.slice #_ #(PS.n) spec_res (v start) (v j)));
//     slice_plus_one #_ #256 #256 (lpoly_v h1 res) spec_res (v start) (v j);
//     assert (Seq.index (lpoly_v h1 res) (v j + v len) == Seq.index #_ #(PS.n) spec_res (v j + v len));
//     assert (Seq.equal (Seq.slice (lpoly_v h1 res) (v start + v len) (v j + v len)) (Seq.slice #_ #(PS.n) spec_res (v start + v len) (v j + v len)));
...
//     assert (Seq.equal (Seq.slice (lpoly_v h1 res) (v start + v len) (v j + 1 + v len)) (Seq.slice #_ #(PS.n) spec_res (v start + v len) (v j + 1 + v len)))
